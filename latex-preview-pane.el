...
(defun latex-preview-pane-update-p ()
  (if (eq (eval `(call-process ,pdf-latex-command nil "*pdflatex-buffer*" nil ,@pdf-latex-command-args ,buffer-file-name)) 1)
...
(defcustom pdf-latex-command-args ()
  "The list of arguments passed to the program pdf-latex-command."
  :type '(repeat string)
  :group 'latex-preview-pane)
